...
function Process(textUser, number){
    ...
    textUser.includes("ola")
        ...
        let model2 = whatsappModel.MessageText("¡Hola y bienvenido(a)!, soy Jessy el bot asistente de Pizza Planeta y estoy aquí para ayudarte en ésta magnífica experiencia.", number);
        models.push(model2);
            
        let modelBuy = whatsappModel.MessageMainMenu(number);
        models.push(modelBuy);
    }
    else if(textUser.includes('ver el menú')) {
        let model = whatsappModel.MessageImage(number);
        models.push(model);
    }
    else if(textUser.includes('realizar pedido')) {
        let model = whatsappModel.MessageText("Empecemos con tu ubicación.\nPuedes mandar *manualmente tu dirección* o *compartir tu ubicación.*", number);
        models.push(model);

        let model2 = whatsappModel.LocationRequired(number);
        models.push(model2);
    }
